...
void GenericTCPServer(void)
{
    ...
    case SM_LISTENING:
        ...
        for (w = 0; w < wMaxGet; w += sizeof (AppBuffer))
        {
            ...
            if (!strncmp(AppBuffer, "@RTCI:", 6))
            {
                RTC_Time tm;
                uint8_t txt[3];
                txt[2] = 0;

                strncpy(txt, &AppBuffer[6], 2);
                tm.hours = (uint8_t) atoi(txt);
                strncpy(txt, &AppBuffer[8], 2);
                tm.minutes = (uint8_t) atoi(txt);
                strncpy(txt, &AppBuffer[10], 2);
                tm.seconds = (uint8_t) atoi(txt);

                strncpy(txt, &AppBuffer[12], 2);
                tm.day = (uint8_t) atoi(txt);
                strncpy(txt, &AppBuffer[14], 2);
                tm.month = (uint8_t) atoi(txt);
                strncpy(txt, &AppBuffer[16], 2);
                tm.year = (uint8_t) atoi(txt);

                strncpy(txt, &AppBuffer[18], 1);
                txt[1] = 0;
                tm.dow = (uint8_t) atoi(txt);

                RTC_Set(&tm);
                // Transfer the data out of our local processing buffer and into the TCP TX FIFO.
                strcpy(txt, "RTC OK\r\n");
                TCPPutArray(MySocket, txt, wCurrentChunk);
            }
            else if (!strncmp(AppBuffer, "@RTCL:", 6))
            {
                RTC_Time tm;
                RTC_Get(&tm);
                uint8_t txt[20];
                ...
                // Transfer the data out of our local processing buffer and into the TCP TX FIFO.
                TCPPutArray(MySocket, txt, wCurrentChunk);
                sprintf(txt, "%02d%02d%02d%d\r\n", tm.day, tm.month, tm.year, tm.dow);
                // Transfer the data out of our local processing buffer and into the TCP TX FIFO.
                TCPPutArray(MySocket, txt, wCurrentChunk);
            }
